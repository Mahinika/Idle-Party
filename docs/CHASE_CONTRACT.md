...
8. **KEY habit (endgame only)** — chase the next KEY until preferred key is at the dial cap (TODAY detail includes affixes + par). Does **not** wait on unpaid Daily. Beats Ascend READY.
9. **Endgame ladder (party Lv100)** — after KEY at cap: week ALMOST (if any) → Gauntlet (PB after F100) → Greater Rift → Rift → Ashen Crown. Month ALMOST after ladder. One hunt — not Daily/Will shuffle. Gauntlet is **not** a 16th PATH cave.
