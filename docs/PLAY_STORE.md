...
| Store listing copy (EN) | ⏳ paste | Default locale **en-US**. Title / short / full in `docs/STORE_LISTING.md` (2026-09-17 crawl-on-screen opening). Live public title still showed **Idle Party** without Idle RPG. Extra locales gated. |
...
| Screenshots + feature graphic | ⏳ paste | **2026-09-17 pack:** shots 1–2 recapture with party-centered camera (`out/01_01_combat_a.png`). Console carousel still the older set until owner attaches. Feature graphic unchanged. Preview rebuild: combat first 10 s (`preview/idle_party_preview_16x9.mp4`); live YT still `OMWXbgGBFMA` until replaced. |
